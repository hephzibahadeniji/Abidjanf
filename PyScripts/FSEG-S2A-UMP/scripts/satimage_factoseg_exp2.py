...
def main():
    ...
    omega = .045
    nct = True
    output_path = out_dir + "%s_factbased_segmented_ws%d_ncT.tif" % (aoi, ws)
    ...
    # # output_feature_filename = out_dir + file_name.split('_')[0] + '_' + "fseg_output_ws25_ncF.npy"
    # np.save(output_feature_filename, seg_out)
    
    # # Save visualized composite texture map
    # output_image_filename = out_dir + "example_M1_output__ws%d_ncT.png" % (ws)
    # output_image_filename = out_dir + "./example_rgb_1_output_ws%d_ncT.png" % (ws)
    # output_image_filename = out_dir + file_name.split('_')[0] + '_' + "fseg_output_ws%d_ncT.png" % (ws)
    output_image_filename = out_dir + "%s_example_S2A_output__ws%d_ncT.png" % (aoi, ws)
